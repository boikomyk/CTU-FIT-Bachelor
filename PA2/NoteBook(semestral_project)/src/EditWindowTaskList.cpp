...
EditWindowTaskList::EditWindowTaskList(int h, int w, int x, int y ): EditWindow(h,w,x,y){}
//---------------------------------------------------------------------------------------

EditWindowTaskList::~EditWindowTaskList(){}

//---------------------------------------------------------------------------------------

void EditWindowTaskList::closeInputAndFree( void )const
{
...
void EditWindowTaskList::initEditField(int linesCnt, int colmCnt){
...
int  EditWindowTaskList::createNote(CSQLManager * mn)
{
    ...
    set_field_buffer(NB_FIELDS[linesCnt - 1][0], 0, (typeName + " " + typeOptions[1]).c_str());
    update_panels();
    refresh();

    
    vector <bool> startContent;
    for(int i = 0; i < linesCnt; i++){
        startContent.push_back(false);
    }
    ...
    while ((key = getch()) != KEY_F(6))
    {
        ...
        switch (key)
        {
            ...
            case KEY_DOWN:
                ...
                else NB_ASK_FORMS[index]->curcol = 2;
                break;
            case KEY_UP:
                if(index == 0)
                    index = (LINES - 9);
            ...
            case 127:
                if((NB_ASK_FORMS[index]->curcol == 0 && index != 0) || (NB_ASK_FORMS[index]->curcol == 6 && index == 1)){
                    ...
                    form_driver(NB_ASK_FORMS[index], key);
                else if(index > 1 && NB_ASK_FORMS[index]->curcol <= 2){
                    index --;
                    form_driver(NB_ASK_FORMS[index], key);
                }
                else
        ...
        if(index > 1 && index < linesCnt - 1 && startContent[index] == false){
            startContent[index] = true;
            set_field_buffer(NB_FIELDS[index][0], 0, "* ");
            NB_ASK_FORMS[index]->curcol = 2;
        }
    ...
    CNote* note = new CNoteTaskList(title, typeOptions[1], buffText, "PlainText", 0);
...
void updateFieldsAfterNotice(const int limit, const int shift, const int curInd, const int curPos){
    for(int i = 9; i < 12; i++){
        if(i > limit)
            break;
        form_driver(NB_ASK_FORMS[i], REQ_DOWN_FIELD);
        form_driver(NB_ASK_FORMS[i + shift], REQ_DOWN_FIELD);
    }
...
void EditWindowTaskList::updateFieldsAfterNotice(const int limit, const int curInd, const int curPos){
...
int EditWindowTaskList::showNote(CSQLManager * mn, CSearchRequest * rq){
    // Define size of active space:
    int linesCnt = LINES - 7;
    int colmCnt  = COLS*3/4 - 4;
    ...
    for(int i = 0; i < (int)content.size(); i++){
        ...
        startContent[2+i] = true;
    }
    mvwprintw(m_Win,1, 1 , (typeName + " " + typeOptions[1]).c_str());
    wrefresh(m_Win);    
